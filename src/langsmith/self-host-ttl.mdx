...
If you do not see a decrease in disk space after running this job, or see disk space increasing, then you may have backups causing file system hard
links, preventing ClickHouse from wiping this data from disk.

To check any current backups, check the `/var/lib/clickhouse/backup` and `/var/lib/clickhouse/shadow` directories inside of your clickhouse pod.

If you see backups in those directories, please copy them over to an external filesystem, or to blob storage such as S3, and wipe those directories.
You should then see disk space freeing over the following minutes.
