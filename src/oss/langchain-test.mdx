...
<Accordion title="Agent with GenericFakeChatModel">
```python
from langchain_core.language_models.fake_chat_models import GenericFakeChatModel
...
:::js
LangChain provides [`FakeListChatModel`](https://v03.api.js.langchain.com/classes/_langchain_core.utils_testing.FakeListChatModel.html) for mocking text responses. It accepts an array of responses and returns one per invocation.
...
If we invoke the model again, it will return the next item in the list:
...
<Accordion title="Agent with FakeChatModel">
```ts
// TODO
```
</Accordion>
