...
You can use [`prompt` parameter](/langchain/agents#prompt) to pass in a function that returns a string to use as system prompt
...
You can use [`prompt` parameter](/langchain/agents#prompt) to pass in a function that returns a list of messages
...
You can use [`prompt` parameter](/langchain/agents#prompt) to pass in a function that returns a list of messages or a custom system prompt.
...
You can use [`prompt` parameter](/langchain/agents#prompt) to pass in a function that returns a list of messages or a custom system prompt.
Session context is passed in with the [`state` parameter](/langchain/short-term-memory#prompt)
...
You can use [pre_model_hook](/langchain/agents#pre-model-hook) to update state
...
You can use `get_runtime` to [access runtime configuration](/langchain/tools#accessing-runtime-context-inside-a-tool) in tools
...
You can add an argument with InjectedState to tools to access [session context in tools](/langchain/short-term-memory#read-short-term-memory-in-a-tool)
...
You can use `get_store` to [access long term memory in tools](/langchain/long-term-memory#read-long-term-memory-in-tools)
...
You can [return state updates](/langchain/short-term-memory#write-short-term-memory-from-tools) with Command from tools
...
You can use `get_store` to access long term memory and then [update it inside tools](/langchain/long-term-memory#write-long-term-memory-from-tools)
...
You can pass in a [function to `model` parameter](/langchain/agents#dynamic-model) that attaches custom tools
...
You can pass in a [function to `model` parameter](/langchain/agents#dynamic-model) that returns a custom model
