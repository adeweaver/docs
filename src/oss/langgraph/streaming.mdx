...
* [**Stream graph state**](/oss/langgraph/streaming#stream-graph-state) — get state updates / values with `updates` and `values` modes.
* [**Stream subgraph outputs**](/oss/langgraph/streaming#stream-subgraph-outputs) — include outputs from both the parent graph and any nested subgraphs.
* [**Stream LLM tokens**](/oss/langgraph/streaming#messages) — capture token streams from anywhere: inside nodes, subgraphs, or tools.
* [**Stream custom data**](/oss/langgraph/streaming#stream-custom-data) — send custom updates or progress signals directly from tool functions.
* [**Use multiple streaming modes**](/oss/langgraph/streaming#stream-multiple-modes) — choose from `values` (full state), `updates` (state deltas), `messages` (LLM tokens + metadata), `custom` (arbitrary user data), or `debug` (detailed traces).
...
:::python
To include outputs from [subgraphs](/oss/subgraphs) in the streamed outputs, you can set `subgraphs=True` in the `.stream()` method of the parent graph. This will stream outputs from both the parent graph and any subgraphs.
