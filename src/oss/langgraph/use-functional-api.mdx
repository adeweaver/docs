...
The functional API supports [human-in-the-loop](/oss/langgraph/human-in-the-loop) workflows using the `interrupt` function and the `Command` primitive.
...
[long-term memory](/oss/langgraph/memory#long-term-memory) allows storing information across different **thread ids**. This could be useful for learning information about a given user in one conversation and using it in another.

## Workflows

* [Workflows and agent](/oss/langgraph/agentic-architectures) guide for more examples of how to build workflows using the Functional API.
