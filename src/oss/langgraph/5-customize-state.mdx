...
<Note>
  This tutorial builds on [Add human-in-the-loop controls](/oss/langgraph/4-human-in-the-loop).
</Note>
...
:::python
Now, populate the state keys inside of the `human_assistance` tool. This allows a human to review the information before it is stored in the state. Use [`Command`](/oss/langgraph/graph-api#using-inside-tools) to issue a state update from inside the tool.
...
:::js
Now, populate the state keys inside of the `humanAssistance` tool. This allows a human to review the information before it is stored in the state. Use [`Command`](/oss/langgraph/graph-api#using-inside-tools) to issue a state update from inside the tool.
...
There's one more concept to review before finishing the LangGraph basics tutorials: connecting `checkpointing` and `state updates` to [time travel](/oss/langgraph/6-time-travel).
