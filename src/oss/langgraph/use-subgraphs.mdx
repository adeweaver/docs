...
This guide explains the mechanics of using [subgraphs](/oss/langgraph/subgraphs). A common application of subgraphs is to build [multi-agent](/oss/langgraph/multi-agent) systems.

When adding subgraphs, you need to define how the parent graph and the subgraph communicate:

* [Shared state schemas](#shared-state-schemas) — parent and subgraph have **shared state keys** in their state [schemas](/oss/langgraph/graph-api#state)
* [Different state schemas](#different-state-schemas) — **no shared state keys** in parent and subgraph [schemas](/oss/langgraph/graph-api#state)
...
A common case is for the parent graph and subgraph to communicate over a shared state key (channel) in the [schema](/oss/langgraph/graph-api#state). For example, in [multi-agent](/oss/langgraph/multi-agent) systems, the agents often communicate over a shared [messages](/oss/langgraph/graph-api#why-use-messages) key.
...
For more complex systems you might want to define subgraphs that have a **completely different schema** from the parent graph (no shared keys). For example, you might want to keep a private message history for each of the agents in a [multi-agent](/oss/langgraph/multi-agent) system.
