...
* How interruptions are resumed in [human-in-the-loop](/oss/langgraph/human-in-the-loop) workflows
* How we can "rewind" and branch-off executions using LangGraph's [time travel](/oss/langgraph/time-travel) features
...
This architecture is similar to a [ReAct agent](/oss/langgraph/agentic-architectures) in which node `"a"` is a tool-calling model, and node `"b"` represents the tools.
