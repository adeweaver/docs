...
LangGraph's [persistence](/oss/langgraph/persistence) layer supports **human-in-the-loop** workflows, allowing execution to pause and resume based on user feedback. The primary interface to this functionality is the [`interrupt`](/oss/langgraph/add-human-in-the-loop) function. Calling `interrupt` inside a node will pause execution. Execution can be resumed, together with new input from a human, by passing in a [Command](/oss/langgraph/graph-api#command).

:::python
`interrupt` is ergonomically similar to Python's built-in `input()`, [with some caveats](/oss/langgraph/add-human-in-the-loop).
:::

:::js
`interrupt` is ergonomically similar to Node.js's built-in `readline.question()` function, [with some caveats](/oss/langgraph/add-human-in-the-loop).
`interrupt` is ergonomically similar to Node.js's built-in `readline.question()` function, [with some caveats](/oss/langgraph/add-human-in-the-loop).
:::

<Note>
  This tutorial builds on [Add memory](/oss/langgraph/3-add-memory).
</Note>

## 1. Add the `human_assistance` tool

Starting with the existing code from the [Add memory to the chatbot](/oss/langgraph/3-add-memory) tutorial, add the `human_assistance` tool to the chatbot. This tool uses `interrupt` to receive information from a human.
...
:::python
```python {highlight={12,19,20,21,22,23}}
...
<Tip>
  For more information and examples of human-in-the-loop workflows, see [Human-in-the-loop](/oss/langgraph/human-in-the-loop).
</Tip>
...
<Info>
  **Additional information**
  ...
  To resume execution, pass a [`Command`](/oss/langgraph/graph-api#command) object containing data expected by the tool. The format of this data can be customized based on needs.
  ...
  1. Look for tutorials or guides specifically focused on building AI agents with LangGraph.
  2. Check if there are any community forums or discussion groups where you can ask questions and get support from other developers using LangGraph.
...
So far, the tutorial examples have relied on a simple state with one entry: a list of messages. You can go far with this simple state, but if you want to define complex behavior without relying on the message list, you can [add additional fields to the state](/oss/langgraph/5-customize-state).
