...
<Note>
  This tutorial builds on [Build a basic chatbot](/oss/langgraph/1-build-basic-chatbot).
</Note>
...
:::python
For the `StateGraph` you created in the [first tutorial](/oss/langgraph/1-build-basic-chatbot), add `bind_tools` on the LLM. This lets the LLM know the correct JSON format to use if it wants to use the search engine.
:::

:::js
For the `StateGraph` you created in the [first tutorial](/oss/langgraph/1-build-basic-chatbot), add `bindTools` on the LLM. This lets the LLM know the correct JSON format to use if it wants to use the search engine.
:::
...
The chatbot cannot remember past interactions on its own, which limits its ability to have coherent, multi-turn conversations. In the next part, you will [add **memory**](/oss/langgraph/3-add-memory) to address this.
