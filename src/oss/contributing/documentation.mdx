...
Documentation is a vital part of LangChain. We welcome both new documentation for new features and integrations, as well as community improvements to our current documentation.
...
LangChain's documentation follows the [**Diataxis framework**](https://diataxis.fr/). Under this framework, all documentation falls under one of four categories:
...
<Steps>
    <Step title="Find the page">
        {/* TODO */}
        Navigate to any documentation page and click the "edit this page" link at the bottom
    </Step>
    ...
    <Step title="Make your changes">
        Correct the typo directly in GitHub's editor
    </Step>
...
For larger changes or ongoing contributions, set up a local development environment:
...
<Steps>
    ...
    <Step title="Development environment">
        Set up your development environment following our [IDE setup guide](#ide-setup)
    </Step>

    <Step title="Understanding the build system">
        Familiarize yourself with our documentation [build pipeline](#build-system)
    </Step>

    <Step title="Mintlify knowledge">
        Review the [Mintlify documentation](https://mintlify.com/docs) for component usage and best practices
    </Step>
</Steps>
...
<Tabs>
    ...
    <Tab title="Other IDEs">
        Most modern IDEs support EditorConfig. The `.editorconfig` file automatically configures:
...
Our documentation uses a build pipeline that converts source files to Mintlify format:

<Steps>
    <Step title="Install dependencies">
        You will need to install [`uv`](https://docs.astral.sh/uv/getting-started/installation/) and [`mint`](https://mintlify.com/docs/installation) if you haven't already. Also, be sure to [clone the docs](#full-development-setup) repo.
...
Conceptual guides fall under the "Explanation" quadrant of Diataxis. They cover LangChain terms and core concepts abstractly, providing deeper understanding.
...
<AccordionGroup>
    ...
    <Accordion title="API reference tips">
        - Be concise but complete
        - Discuss special cases and deviations from expectations
        - Detail required inputs and outputs
...
- [LangChain JavaScript/TypeScript API reference](https://v03.api.js.langchain.com/index.html)
...
<Note>
    We don't want a lack of parity to block contributions. If a feature is only available in one language, it's okay to have documentation only in that language until the other language catches up.
</Note>
<Note>
