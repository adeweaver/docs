...
* <Icon icon="hammer" size={16} /> [Tool calling](#tool-calling) - enables models to interact with external
services, APIs, and databases.
* <Icon icon="shapes" size={16} /> [Structured output](#structured-output) - constrain the response format to
match a provided schema.
* <Icon icon="image" size={16} /> [Multimodality](#multimodality) - the ability to process data other than text,
such as images.
* <Icon icon="brain" size={16} /> [Reasoning](#reasoning) - the ability to perform multi-step reasoning to
...
`init_chat_model()` to initialize one from a model [provider](/oss/integrations/providers) of
your choice.
...
See the `init_chat_model` [API reference](https://python.langchain.com/api_reference/langchain/chat_models/langchain.chat_models.base.init_chat_model.html)
for more detail.
...
Importantly, each `AIMessageChunk` in a stream is designed to be gathered into a full
message via summation:
...
The resulting full message can be treated the same as a message that was generated
with [invoke](#invoke)— for example, it can be aggregated into a message history
and passed back to the model as conversational context.
...
LangChain chat models can also stream semantic events using the
...
This simplifies filtering based on event types and other metadata, and will aggregate
the full message in the background. See below for an example.

<Accordion title="Example: stream events">
```python
...
See the [API reference](https://python.langchain.com/api_reference/core/language_models/langchain_core.language_models.chat_models.BaseChatModel.html#langchain_core.language_models.chat_models.BaseChatModel.astream_events)
for event types and other details.
...
Each model has a <Tooltip tip="The maximum number of tokens a model can process in a single request, including both input and output tokens">context window</Tooltip>
that determines how much text it can process at once. To learn more, see our
guide on [context engineering]() for strategies to effectively manage context.
...
<Tabs>
    <Tab title="Pydantic">
        ...
        descriptions, and nested structures.
    ...
    <Tab title="JSON Schema">
        ...
        Schema. Some providers support `strict=True` for guaranteed schema
        adherence.
        ...
        model_with_structure = model.with_structured_output(
            json_schema, 
            method="json_schema",
            strict=True  # Available on certain providers
        )
        response = model_with_structure.invoke("Provide details about the movie Inception")
...
<Note>
    **Key considerations for structured outputs:**
    ...
    - **Strict mode**: When available, `strict=True` guarantees the output will
...
<Accordion title="Advanced usage with nested structures">
    ```python
    from pydantic import BaseModel, Field
    ...
    model_with_structure = model.with_structured_output(
        MovieDetails, 
        include_raw=True  # Returns both parsed and raw response
    )

    result = model_with_structure.invoke("Tell me about The Matrix movie")
    print(result["parsed"])  # Access the structured data
    print(result["raw"])     # Access the original AI message
    ```
</Accordion>

<Accordion title="Structured output with tool calling">
    You can combine structured outputs with tool calling to ensure tools return
    data in a specific format. This is particularly useful when tools need to
    extract or process information in a standardized way.
    ...
    @tool(response_format=SearchResult)
    ...
    # The tool will automatically return structured data
    response = model_with_tools.invoke("Search for information about quantum computing")
    if response.tool_calls:
        tool_result = response.tool_calls[0]
        print(tool_result["args"])  # Structured as SearchResult
    ```

    You can also use `with_structured_output()` alongside tools for extracting
    structured information from tool responses:
    ...
    # Create a model that uses tools and outputs structured data
    model_with_tools = model.bind_tools([web_search])
    structured_model = model_with_tools.with_structured_output(Analysis)

    # This will:
    # 1. Call the search tool if needed
    # 2. Process the results
    # 3. Return structured analysis
    result = structured_model.invoke(
        "Search for quantum computing and analyze the findings"
    )
    print(result)  # Analysis(summary="...", key_points=[...], needs_more_info=False)
    ```
</Accordion>
...
`config` parameter using a [`RunnableConfig`]() dictionary. This provides
...
You can also create a runtime-configurable model by specifying `configurable_fields`.
If you don't specify a model value, then "model" and "model_provider" be configurable by default.
...
We can create a configurable model with default model values, specify which parameters
are configurable, and add prefixes to configurable params:
```python
...
`with_configurable`, etc. on a configurable model and chain a configurable model in the
same way that we would a regularly instantiated chat model object.
```python
from pydantic import BaseModel, Field
