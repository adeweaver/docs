...
type HumanInterruptConfig = {
  allow_ignore: boolean;  // Allow skipping this email
  allow_respond: boolean; // Allow text feedback
  allow_edit: boolean;    // Allow editing email fields
  allow_accept: boolean;  // Allow direct approval
};
...
type InterruptResponse =
  | { type: "accept"; args: ActionRequest["args"] }
  | { type: "edit"; args: ActionRequest["args"] }
  | { type: "respond"; args: string }
  | { type: "ignore" };

function create_email_agent() {
  async function send_email(to: string, subject: string, body: string) {
    ...
    const response = (await interrupt<InterruptResponse[]>(request))[0];

    console.log("RESPONSE", response);

    if (response?.type === "accept") {
      const action_request = response.args; // dict
      // ... send email
    } else if (response?.type === "edit") {
      const edited_action_request = response.args; // dict
      // ... send email using edited values
    } else if (response?.type === "respond") {
      const feedback = response.args; // str
      // process the human feedback
  ...
  const send_email_tool = tool({
    name: "send_email",
    description: "Send an email after human approval.",
    schema: z.object({
      to: z.string(),
      subject: z.string(),
      body: z.string(),
    ...
    func: async ({ to, subject, body }) => {
      ...
      return "Email handled (pending human interrupt).";
    },
  });
...
const agent = create_email_agent();
```
:::
