...
<Note>
This overview focuses on text‑based embedding models. LangChain does not currently support multimodal embedding models.
</Note>

Embedding models convert raw text—such as a tweet, sentence, or paragraph—into a fixed-length vector of numbers that captures the text’s **semantic meaning**. These vector representations enable machines to perform **semantic comparisons** and **search operations** that go beyond simple keyword matching.

For example, instead of finding documents containing the exact phrase "machine learning," embeddings let systems find documents that discuss related concepts—even if they use different language.

### How it works

At a high level, the process involves two steps:

1. **Embed text as a vector**: The model transforms each input string into a high-dimensional vector.
2. **Measure similarity**: These vectors can then be compared using mathematical similarity metrics.
...
LangChain provides a standard interface for text embedding models (e.g., OpenAI, Cohere, Hugging Face) via the @[`Embeddings`] class.
...
Some providers **may** use distinct models or strategies for queries versus documents, which is why these methods are separate.
...
In production, you would typically use a more robust persistent store, such as a database or cloud storage.

You can either roll out a custom `ByteStore` or use one of the built-in options:



## Swapping the `ByteStore`

To use a different `ByteStore`, pass it to `CacheBackedEmbeddings` at initialization.
For example, here we create an equivalent cached embeddings object using the non-persistent `InMemoryByteStore`:
