...
<Note>
Requires running Helm chart version 0.11.4 or later.
</Note>

### Initial export

<Note>
These scripts export usage data to a CSV for reporting to LangChain.
They additionally track the export by assigning a backfill ID and timestamp.
</Note>

To export LangSmith trace usage:

```bash
sh run_support_query_pg.sh <postgres_url> --input support_queries/postgres/pg_usage_traces_backfill_export.sql --output export.csv
```

To export LangGraph Platform usage:

```bash
sh run_support_query_pg.sh <postgres_url> --input support_queries/postgres/pg_usage_nodes_backfill_export.sql --output export.csv
```
...
```bash
sh run_support_query_pg.sh <postgres_url> --input support_queries/postgres/pg_usage_traces_backfill_update.sql --output export.csv
```

To update LangGraph Platform usage:

```bash
sh run_support_query_pg.sh <postgres_url> --input support_queries/postgres/pg_usage_nodes_backfill_update.sql --output export.csv
```
