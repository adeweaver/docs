...
* *Note*: Langsmith SSO is only supported through `https`.
...
Once service-provide-initiated SSO is configured, users can sign in using the **Login via SSO** button in the LangSmith UI.
...
This flow does *not* require a `Client Secret` - see the flow [above](#with-secret) for the alternative that does.
