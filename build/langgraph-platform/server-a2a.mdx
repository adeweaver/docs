...
Agent-to-Agent (A2A) is Google's protocol for enabling communication between conversational AI agents. LangGraph Platform implements A2A support, allowing your agents to communicate with other A2A-compatible agents through a standardized protocol.

[LangGraph Server](/langgraph-platform/langgraph-server) implements the A2A protocol, enabling **agents** to communicate with each other using structured message exchanges. This allows for multi-agent conversations and collaborative workflows.

The A2A endpoint is available at `/a2a/{assistant_id}` on [LangGraph Server](/langgraph-platform/langgraph-server).
...
Your agent needs to handle message-based conversations. Here's a simple example:
...
Once your agents are deployed, you can facilitate communication between them using the A2A protocol:
