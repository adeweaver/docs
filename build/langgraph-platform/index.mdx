...
* **[Checkpointers and memory management](/oss/python/langgraph/persistence#checkpoints)**: For agents needing persistence (e.g., conversation memory), deploying a robust storage solution can be complex. LangGraph Platform includes optimized [checkpointers](/oss/python/langgraph/persistence#checkpoints) and a [memory store](/oss/python/langgraph/persistence#memory-store), managing state across sessions without the need for custom solutions.
