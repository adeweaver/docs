...
<Note>
  This tutorial builds on [Add human-in-the-loop controls](/langgraph-platform/langgraph-basics/4-human-in-the-loop).
</Note>
...
Now, populate the state keys inside of the `human_assistance` tool. This allows a human to review the information before it is stored in the state. Use [`Command`](/oss/python/langgraph/graph-api#using-inside-tools) to issue a state update from inside the tool.
...
There's one more concept to review before finishing the LangGraph basics tutorials: connecting `checkpointing` and `state updates` to [time travel](/langgraph-platform/langgraph-basics/6-time-travel).
