...
To use [time-travel](/oss/javascript/time-travel) in LangGraph:
...
2. [Identify a checkpoint in an existing thread](#2-identify-a-checkpoint): Use the [`getStateHistory()`](https://langchain-ai.github.io/langgraphjs/reference/classes/langgraph.CompiledStateGraph.html#getStateHistory) method to retrieve the execution history for a specific `thread_id` and locate the desired `checkpoint_id`.  
  Alternatively, set a [breakpoint](/oss/javascript/add-human-in-the-loop) before the node(s) where you want execution to pause. You can then find the most recent checkpoint recorded up to that breakpoint.
...
<Tip>
  For a conceptual overview of time-travel, see [Time travel](/oss/javascript/time-travel).
</Tip>
