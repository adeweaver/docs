...
In a typical chatbot workflow, the user interacts with the bot one or more times to accomplish a task. [Memory](/oss/javascript/langgraph/3-add-memory) and a [human-in-the-loop](/oss/javascript/langgraph/4-human-in-the-loop) enable checkpoints in the graph state and control future responses.
...
<Note>
  This tutorial builds on [Customize state](/oss/javascript/langgraph/5-customize-state).
</Note>
