...
LangGraph comes with a set of [prebuilt components](/oss/javascript/langgraph/prebuilts) that implement common agent behaviors and architectures. These abstractions are built on top of the LangGraph framework, offering a faster path to production. These components encapsulate complex orchestration logic into simple, configurable interfaces.
...
[Low-level orchestration](/oss/javascript/langgraph/why-langgraph) provides complete control over agent behavior through custom graph construction. You define the exact flow, state management, and decision logic for your agent or workflow.
...
For more information, see [Graph API](/oss/javascript/langgraph/graph-api).
