...
<Note>
  This tutorial builds on [Add human-in-the-loop controls](/oss/javascript/langgraph/4-human-in-the-loop).
</Note>
...
Now, populate the state keys inside of the `humanAssistance` tool. This allows a human to review the information before it is stored in the state. Use [`Command`](/oss/javascript/langgraph/graph-api#using-inside-tools) to issue a state update from inside the tool.
...
There's one more concept to review before finishing the LangGraph basics tutorials: connecting `checkpointing` and `state updates` to [time travel](/oss/javascript/langgraph/6-time-travel).
