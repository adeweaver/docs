...
You can [stream outputs](/oss/javascript/langgraph/streaming) from a LangGraph agent or workflow.
...
In some applications you might need to disable streaming of individual tokens for a given model. This is useful in [multi-agent](/oss/javascript/langgraph/multi-agent) systems to control which agents stream their output.

See the [Models](/oss/javascript/langgraph/models#disable-streaming) guide to learn how to disable streaming.
...
To include outputs from [subgraphs](/oss/javascript/langgraph/subgraphs) in the streamed outputs, you can set `subgraphs: true` in the `.stream()` method of the parent graph. This will stream outputs from both the parent graph and any subgraphs.
