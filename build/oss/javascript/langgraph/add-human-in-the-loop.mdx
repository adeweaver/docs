...
To review, edit, and approve tool calls in an agent or workflow, use interrupts to pause a graph and wait for human input. Interrupts use LangGraph's [persistence](/oss/javascript/persistence) layer, which saves the graph state, to indefinitely pause graph execution until you resume.

<Info>
  For more information about human-in-the-loop workflows, see the [Human-in-the-Loop](/oss/javascript/human-in-the-loop) conceptual guide.
</Info>
...
2. The `MemorySaver` is used to store the agent state at every step in the tool calling loop. This enables [short-term memory](/oss/javascript/add-memory#add-short-term-memory) and [human-in-the-loop](/oss/javascript/human-in-the-loop) capabilities. In this example, we use `MemorySaver` to store the agent state in memory. In a production application, the agent state will be stored in a database.
