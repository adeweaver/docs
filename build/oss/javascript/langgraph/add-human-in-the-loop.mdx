...
<Info>
  For more information about human-in-the-loop workflows, see the [Human-in-the-Loop](/oss/javascript/langgraph/human-in-the-loop) conceptual guide.
</Info>
...
2. The `MemorySaver` is used to store the agent state at every step in the tool calling loop. This enables [short-term memory](/oss/javascript/langgraph/add-memory#add-short-term-memory) and [human-in-the-loop](/oss/javascript/langgraph/human-in-the-loop) capabilities. In this example, we use `MemorySaver` to store the agent state in memory. In a production application, the agent state will be stored in a database.
