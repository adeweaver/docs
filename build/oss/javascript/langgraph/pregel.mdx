...
LangGraph provides two high-level APIs for creating a Pregel application: the [StateGraph (Graph API)](/oss/javascript/graph-api) and the [Functional API](/oss/javascript/functional-api).
...
<Tabs>
  ...
  <Tab title="Functional API">
    ...
    In the [Functional API](/oss/javascript/functional-api), you can use an [`entrypoint`](https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.entrypoint.html) to create a Pregel application. The `entrypoint` decorator allows you to define a function that takes input and returns output.
