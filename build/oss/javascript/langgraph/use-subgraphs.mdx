...
This guide explains the mechanics of using [subgraphs](/oss/javascript/subgraphs). A common application of subgraphs is to build [multi-agent](/oss/javascript/multi-agent) systems.

When adding subgraphs, you need to define how the parent graph and the subgraph communicate:

* [Shared state schemas](#shared-state-schemas) — parent and subgraph have **shared state keys** in their state [schemas](/oss/javascript/graph-api#state)
* [Different state schemas](#different-state-schemas) — **no shared state keys** in parent and subgraph [schemas](/oss/javascript/graph-api#state)
...
A common case is for the parent graph and subgraph to communicate over a shared state key (channel) in the [schema](/oss/javascript/graph-api#state). For example, in [multi-agent](/oss/javascript/multi-agent) systems, the agents often communicate over a shared [messages](/oss/javascript/graph-api#why-use-messages) key.
...
For more complex systems you might want to define subgraphs that have a **completely different schema** from the parent graph (no shared keys). For example, you might want to keep a private message history for each of the agents in a [multi-agent](/oss/javascript/multi-agent) system.
...
When you enable [persistence](/oss/javascript/persistence), you can [inspect the graph state](/oss/javascript/persistence#checkpoints) (checkpoint) via the appropriate method. To view the subgraph state, you can use the subgraphs option.
