...
AI applications need [memory](/oss/javascript/langgraph/memory) to share context across multiple interactions. In LangGraph, you can add two types of memory:
...
If your graph contains [subgraphs](/oss/javascript/langgraph/subgraphs), you only need to provide the checkpointer when compiling the parent graph. LangGraph will automatically propagate the checkpointer to the child subgraphs.
...
See the [Context](/oss/javascript/langgraph/context) guide for more information.
...
<Tabs>
  ...
  <Tab title="In a workflow">
    ...
    Prompting and orchestration logic can be used to summarize the message history. For example, in LangGraph you can extend the [`MessagesZodState`](/oss/javascript/langgraph/graph-api#working-with-messages-in-graph-state) to include a `summary` key:
