...
If you are just getting started with agents, or want a higher level abstraction, it is recommended that you use [LangChain's agents](/langchain/agents) to get started.
...
* [Durable execution](/oss/javascript/langgraph/durable-execution): Build agents that persist through failures and can run for extended periods, automatically resuming from exactly where they left off.
* [Human-in-the-loop](/oss/javascript/langgraph/human-in-the-loop): Seamlessly incorporate human oversight by inspecting and modifying agent state at any point during execution.
* [Comprehensive memory](/oss/javascript/langgraph/memory): Create truly stateful agents with both short-term working memory for ongoing reasoning and long-term persistent memory across sessions.
...
* [LangChain](https://python.langchain.com/docs/introduction/) – Provides integrations and composable components to streamline LLM application development. Also contains agent abstractions built on top of LangGraph.
