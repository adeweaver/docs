...
The [**Functional API**](/oss/javascript/functional-api) allows you to add LangGraph's key features — [persistence](/oss/javascript/persistence), [memory](/oss/javascript/add-memory), [human-in-the-loop](/oss/javascript/human-in-the-loop), and [streaming](/oss/javascript/streaming) — to your applications with minimal changes to your existing code.

<Tip>
  For conceptual information on the functional API, see [Functional API](/oss/javascript/functional-api).
</Tip>
...
The **Functional API** and the [**Graph API**](/oss/javascript/graph-api) can be used together in the same application as they share the same underlying runtime.
...
read the [**streaming guide**](/oss/javascript/streaming) section for more details.
...
The functional API supports [human-in-the-loop](/oss/javascript/human-in-the-loop) workflows using the `interrupt` function and the `Command` primitive.

### Basic human-in-the-loop workflow

We will create three [tasks](/oss/javascript/functional-api#task):
...
We can now compose these tasks in an [entrypoint](/oss/javascript/functional-api#entrypoint):
...
[interrupt()](/oss/javascript/add-human-in-the-loop#pause-using-interrupt) is called inside a task, enabling a human to review and edit the output of the previous task. The results of prior tasks-- in this case `step_1`-- are persisted, so that they are not run again following the `interrupt`.
...
Note that we've paused with an `interrupt` after `step_1`. The interrupt provides instructions to resume the run. To resume, we issue a [Command](/oss/javascript/add-human-in-the-loop#resume-using-the-command-primitive) containing the data expected by the `human_feedback` task.
...
To review tool calls before execution, we add a `review_tool_call` function that calls [`interrupt`](/oss/javascript/add-human-in-the-loop#pause-using-interrupt). When this function is called, execution will be paused until we issue a command to resume it.
...
Short-term memory allows storing information across different **invocations** of the same **thread id**. See [short-term memory](/oss/javascript/functional-api#short-term-memory) for more details.
...
[long-term memory](/oss/javascript/memory#long-term-memory) allows storing information across different **thread ids**. This could be useful for learning information about a given user in one conversation and using it in another.

## Workflows

* [Workflows and agent](/oss/javascript/agentic-architectures) guide for more examples of how to build workflows using the Functional API.
