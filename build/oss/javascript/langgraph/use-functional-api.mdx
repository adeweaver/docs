...
The [**Functional API**](/oss/javascript/langgraph/functional-api) allows you to add LangGraph's key features — [persistence](/oss/javascript/langgraph/persistence), [memory](/oss/javascript/langgraph/add-memory), [human-in-the-loop](/oss/javascript/langgraph/human-in-the-loop), and [streaming](/oss/javascript/langgraph/streaming) — to your applications with minimal changes to your existing code.
...
The functional API supports [human-in-the-loop](/oss/javascript/langgraph/human-in-the-loop) workflows using the `interrupt` function and the `Command` primitive.
...
[long-term memory](/oss/javascript/langgraph/memory#long-term-memory) allows storing information across different **thread ids**. This could be useful for learning information about a given user in one conversation and using it in another.

## Workflows

* [Workflows and agent](/oss/javascript/langgraph/agentic-architectures) guide for more examples of how to build workflows using the Functional API.
