...
<Note>
  This tutorial builds on [Build a basic chatbot](/oss/javascript/langgraph/1-build-basic-chatbot).
</Note>
...
For the `StateGraph` you created in the [first tutorial](/oss/javascript/langgraph/1-build-basic-chatbot), add `bindTools` on the LLM. This lets the LLM know the correct JSON format to use if it wants to use the search engine.
...
The chatbot cannot remember past interactions on its own, which limits its ability to have coherent, multi-turn conversations. In the next part, you will [add **memory**](/oss/javascript/langgraph/3-add-memory) to address this.
