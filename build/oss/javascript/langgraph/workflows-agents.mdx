...
This guide reviews common workflow and agent patterns. Workflows have predetermined code paths and are designed to operate in a certain order. Agents are more dynamic and define their own processes and tool usage.
...
You can use [any chat model](/oss/javascript/integrations/chat) that supports structured outputs and tool calling to build workflows and agents. To get started, install dependencies and then set your API keys like the Anthropic example below.
...
Prompt chaining is when each LLM call processes the output of the previous call. It's often used for performing well-defined tasks that can be broken down into smaller, verifiable steps. Some examples include translating documents into different languages or verifying generated content for consistency.
...
Routing workflows process inputs and then directs them to context-specific tasks. This allows you to define specialized flows, and should be used for complex tasks. For example, a workflow built to answer product related questions might process the type of question first, and then route the request to specific processes for pricing, refunds, returns, etc.
...
Orchestrator-worker configurations provide more flexibility and are often used when subtasks can't be predefined the way they can with [parallelization](#parallelization). This is common with workflows that write code or need to update content across multiple files. For example, a workflow that needs to update installation instructions for multiple Python libraries across an unknown number of documents might use this pattern.
