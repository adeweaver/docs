...
First, set up LangGraph API server [locally](/oss/javascript/langgraph/local-server) or deploy your agent on [LangGraph Platform](/langgraph-platform/quick-start-studio).
...
Agent Chat UI has full support for [human-in-the-loop](/oss/javascript/langgraph/human-in-the-loop) workflows. To try it out, replace the agent code in `src/agent/graph.py` (from the [deployment](/oss/javascript/langgraph/local-server) guide) with this [agent implementation](/oss/javascript/langgraph/add-human-in-the-loop#add-interrupts-to-any-tool).

<Warning>
  Agent Chat UI works best if your LangGraph agent interrupts using the [`HumanInterrupt` schema](https://langchain-ai.github.io/langgraphjs/reference/interfaces/langgraph_prebuilt.HumanInterrupt.html). If you do not use that schema, the Agent Chat UI will be able to render the input passed to the `interrupt` function, but it will not have full support for resuming your graph.
</Warning>
