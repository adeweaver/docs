...
Diffbot's Extract API requires an API token. Follow these instructions to [get a free API token](/oss/javascript/integrations/providers/diffbot#installation-and-setup) and then set an environment variable.
...
To continue loading the data into a Knowledge Graph, follow the [`DiffbotGraphTransformer` guide](/oss/javascript/integrations/graphs/diffbot/#loading-the-data-into-a-knowledge-graph).
