...
<Warning>
**The Anthropic API officially supports tool-calling so this workaround is no longer needed. Please use [ChatAnthropic](/oss/javascript/integrations/chat/anthropic) with `langchain-anthropic>=0.1.15`.**
...
`ChatAnthropicTools` also implements the [`with_structured_output` spec](/oss/javascript/how-to/structured_output) for extracting values. Note: this may not be as stable as with models that explicitly offer tool calling.
