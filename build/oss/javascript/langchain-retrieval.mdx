...
* **Finite context** — they can’t ingest entire corpora at once as input.
...
👉 [**Tutorial**: Agentic RAG with Self-Correction](https://langchain-ai.github.io/langgraph/tutorials/rag/langgraph_agentic_rag)

## Building a knowledge base

A **knowledge base** is a repository of documents or data you query during retrieval. 
...
**PLACEHOLDER FOR QUICKSTART** (show a full pipeline in a few lines of code)


👉 [**Tutorial**: Agentic RAG with Self-Correction](https://js.langchain.com/docs/tutorials/rag/)
...
Ingest data from external sources (Google Drive, Slack, Notion, etc.). They implement `.load()` and return standardized `Document` objects.

👉 [See integrations](/oss/javascript/integrations/document_loaders)

#### Text splitters

Break large docs into smaller chunks that will be retrievable individually and fit within model context window limit.


👉 [Splitter how-tos](https://js.langchain.com/docs/how_to/#text-splitters)
...
👉 [Embedding integrations](/oss/javascript/integrations/text_embedding)

#### Vector stores

Specialized databases for storing and searching embeddings.

👉 [Vector store integrations](/oss/javascript/integrations/vectorstores/)

#### Retrievers

A [retriever](/oss/javascript/python/concepts/retrievers) is an interface that returns documents given an unstructured query.
...
Retrievers can be created from vector stores, but are also broad enough to include [Wikipedia search](/oss/javascript/python/integrations/retrievers/wikipedia/) and [Amazon Kendra](/oss/javascript/python/integrations/retrievers/amazon_kendra_retriever/).
...
<Tip>
All [vector stores](/oss/javascript/python/concepts/vectorstores) can be [cast to retrievers](/oss/javascript/python/how-to/vectorstore_retriever/).
</Tip>

👉 [Retriever integrations](/oss/javascript/integrations/retrievers/)
