...
### Preserving simplicity while solving real problems
...
Standard content blocks don't change what models can do—they ensure that **identical capabilities are represented identically** across providers. This means developers can access the full power of modern LLMs without sacrificing the portability that makes LLM applications maintainable.

## What are content blocks?

Standard content blocks are a comprehensive set of typed data structures that normalize content across all LLM providers. At a glance, they include:
...
You can control the format written to `.content` using the `output_version` parameter.

This is useful for applications that need to serialize the new blocks.
...
**Ready to try it?** Check out our [migration guide](/oss/javascript/langchain-migrations-v1-0-0).
