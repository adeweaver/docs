...
The easiest way to get started with a model in LangChain is to use `initChatModel` to initialize one from a model [provider](/oss/javascript/integrations/providers) of your choice.
...
<Accordion title='"Auto-Streaming" Chat Models'>
    ...
    Callback events allow LangGraph `stream()` and @[streamEvents][`streamEvents()`]
...
<Accordion title="Stream events">
    ...
    See the @[streamEvents][`streamEvents()`] reference for event types and other details.
...
To make tools that you have defined available for use by a model, you must bind them using @[BaseChatModel.bindTools][`bindTools()`]. In subsequent invocations, the model can choose to call any of the bound tools as needed.
...
<CodeGroup>
    ...
    ```typescript Full reasoning output
    const response = await model.invoke("Why do parrots have colorful feathers?");
