...
to initialize one from a [provider](/oss/javascript/integrations/providers) of your choice.
...
respective provider's [reference docs](/oss/javascript/integrations/providers).
</Info>
...
[`invoke()`](https://v03.api.js.langchain.com/classes/_langchain_core.language_models_chat_models.BaseChatModel.html#invoke) with a single message or a list of messages.
...
conversation. See the [messages](/oss/javascript/langchain-messages) guide for more detail
...
Calling [`stream()`](https://v03.api.js.langchain.com/classes/_langchain_core.language_models_chat_models.BaseChatModel.html#stream) returns an <Tooltip tip="An object that progressively provides access to each item of a collection, in order.">iterator</Tooltip>
...
As opposed to `invoke()`, which returns a single [`AIMessage`](https://v03.api.js.langchain.com/classes/_langchain_core.messages_ai_message.AIMessage.html) after
the model has finished generating its full response, `stream()` returns multiple
[`AIMessageChunk`](https://v03.api.js.langchain.com/classes/_langchain_core.messages_ai_message.AIMessageChunk.html) objects, each containing a portion of the
output text.
...
<Accordion title="Advanced streaming topics">
    <Accordion title='"Auto-streaming" chat models'>
        ...
        In [LangGraph agents](/oss/javascript/langchain-agents), for example, you can
        ...
        [`streamEvents()`](https://v03.api.js.langchain.com/classes/_langchain_core.language_models_chat_models.BaseChatModel.html#streamEvents) to surface the chat
    ...
    See the [`streamEvents()`](https://v03.api.js.langchain.com/classes/_langchain_core.language_models_chat_models.BaseChatModel.html#streamEvents) reference for event
...
<Tip>
    ...
    See the [`RunnableConfig`](https://v03.api.js.langchain.com/interfaces/_langchain_core.runnables.RunnableConfig.html) reference for a full list of
...
them using [`bindTools()`](https://v03.api.js.langchain.com/classes/langchain.chat_models_universal.ConfigurableModel.html#bindTools). In subsequent invocations,
the model can choose to call any of the bound tools as needed.


Some model providers offer built-in tools that can be enabled via model
parameters. Check the respective [provider reference](/oss/javascript/integrations/providers)
for details.

<Tip>
    See the [tools guide](/oss/javascript/langchain-tools) for details and other options
    for creating tools.
</Tip>
...
[content blocks](/oss/javascript/langchain-messages#content).
...
See the [integrations page](/oss/javascript/integrations/providers) for details on specific
providers.
...
[Ollama](/oss/javascript/integrations/providers/ollama) is one of the easiest ways to run
...
<Accordion title="Base URL" icon="link">
    ...
    <Note>
        ...
        [reference](/oss/javascript/integrations/providers) for details.
    </Note>
</Accordion>
...
the [messages](/oss/javascript/langchain-messages) guide.
...
`config` parameter using a [`RunnableConfig`](https://v03.api.js.langchain.com/interfaces/_langchain_core.runnables.RunnableConfig.html) object. This
...
[`RunnableConfig`](https://v03.api.js.langchain.com/interfaces/_langchain_core.runnables.RunnableConfig.html) reference.
