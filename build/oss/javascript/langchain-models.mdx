...
The primary interface for interacting with models in LangChain are the chat
model abstractions. They provide a consistent interface for accessing different
LLMs from various providers.

The easiest way start interacting with a model in LangChain is to use
`init_chat_model()` to initialize one from a model [provider](/oss/javascript/integrations/providers) of
your choice.
...
See [init_chat_model](https://v03.api.js.langchain.com/functions/langchain.chat_models_universal.initChatModel.html) for more detail.
...
<Info>
    ...
    [Integrations page](/oss/javascript/integrations/providers) for details.
</Info>
...
<ParamField body="api_key" type="string">
    ...
    usually issued when you sign up for access to the model. Can often be accessed from
...
<Accordion title='"Auto-Streaming" Chat Models'>
    ...
    In [LangGraph agents](/oss/javascript/python/langchain-agents), for example, you can call
    `model.invoke` within nodes, but LangChain will automatically delegate to streaming
    if running in a streaming mode.
...
[astream_events](https://v03.api.js.langchain.com/types/_langchain_core.tracers_log_stream.StreamEvent.html) API.
...
See the [astream_events](https://v03.api.js.langchain.com/types/_langchain_core.tracers_log_stream.StreamEvent.html) API reference for event types and other details.

### Batch

<Note>

This section describes a chat model method `.batch`, which parallelizes model calls
client-side. It is distinct from batch APIs supported by inference providers.
...
1. A schema, including the name of the tool, a description, and/or argument definitions (often
defined with something like JSON schema)
...
parameters. Check the respective [provider reference](/oss/javascript/integrations/providers) for
details.

<Tip>

See the [tools guide](/oss/javascript/langchain-tools) for details and other options for creating
tools.
...
The LangGraph framework simplifies the orchestration of model and tool calls. See the
[Agent guides](/oss/javascript/langchain-agents) to get started. Below, we show some simple
...
<Accordion title="Example: get AIMessage output alongside parsed structure">

It can be useful to return the raw `AIMessage` object alongside the parsed
representation, to access response metadata such as token counts and other information.
...
encapsulate the data. See examples below.
...
Some models can also return multimodal data as part of their response.

In these cases, the resulting `AIMessage.content_blocks` will contain blocks with
type `"image`.

See the [Integrations page](/oss/javascript/integrations/providers) for details on your
specific providers.
...
[Integrations page](/oss/javascript/integrations/providers).
...
[messages](/oss/javascript/python/langchain-messages) guide.

<Note>

Some provider APIs, notably OpenAI and Azure OpenAI chat completions, require users
opt-in to receiving token usage data in streaming contexts. See
[this section](/oss/javascript/integrations/chat/openai#streaming-usage-metadata) of the
