...
Modern LLMs are primarily accessed through a chat interface, where
[messages](./langchain-messages.mdx) are exchanged between a human user and the
model. In LangChain, models can be interacted with programatically to build
complex, stateful applications.

In addition to basic text generation, many models support the following
features:

* <Icon icon="hammer" size={16} /> [Tool calling](#tool-calling) - enables
models to interact with external services, APIs, and databases.
* <Icon icon="shapes" size={16} /> [Structured output](#structured-outputs) -
constrain the response format to match a provided schema.
* <Icon icon="image" size={16} /> [Multimodality](#multimodality) - the ability
to process data other than text, such as images.
* <Icon icon="brain" size={16} /> [Reasoning](#reasoning) - the ability to
...
`init_chat_model()` to initialize one from a model
[provider](/oss/javascript/integrations/providers) of your choice.
...
<Tip>
    ...
    A full list of async methods can be found in the [reference](https://python.langchain.com/api_reference/core/language_models/langchain_core.language_models.chat_models.BaseChatModel.html#langchain_core.language_models.chat_models.BaseChatModel.invoke).
</Tip>

### Invoke

The most straightforward way to call a model is to use [`invoke()`](https://python.langchain.com/api_reference/core/language_models/langchain_core.language_models.chat_models.BaseChatModel.html#langchain_core.language_models.chat_models.BaseChatModel.invoke)
...
sequence. See the [messages](./langchain-messages.mdx) guide for more detail on
roles, types, and content.
...
Calling [`stream()`](https://python.langchain.com/api_reference/core/language_models/langchain_core.language_models.chat_models.BaseChatModel.html#langchain_core.language_models.chat_models.BaseChatModel.stream)
returns an <Tooltip tip="An object that progressively provides access to each item of a collection, in order.">iterator</Tooltip>
...
The resulting full message can be treated the same as a message that was
...
<Accordion title='"Auto-Streaming" Chat Models'>
    ...
    In [LangGraph agents](/oss/javascript/python/langchain-agents), for example, you can
    ...
    Callback events allow LangGraph `stream()` and [`astream_events()`](https://python.langchain.com/api_reference/langchain/agents/langchain.agents.agent.AgentExecutor.html#langchain.agents.agent.AgentExecutor.astream_events)
...
>    
    ...
    See the [`astream_events()`](https://python.langchain.com/api_reference/langchain/agents/langchain.agents.agent.AgentExecutor.html#langchain.agents.agent.AgentExecutor.astream_events)
...
By default, [`batch()`](https://python.langchain.com/api_reference/core/language_models/langchain_core.language_models.chat_models.BaseChatModel.html#langchain_core.language_models.chat_models.BaseChatModel.batch)
...
stream results with [`batch_as_completed()`](https://python.langchain.com/api_reference/core/language_models/langchain_core.language_models.chat_models.BaseChatModel.html#langchain_core.language_models.chat_models.BaseChatModel.batch_as_completed):
...
<Tip>
    ...
    See the [`RunnableConfig`](https://python.langchain.com/api_reference/core/runnables/langchain_core.runnables.config.RunnableConfig.html#runnableconfig)
...
guide on [context engineering](./langchain-context-engineering.mdx) for strategies to effectively manage context.
...
definitions (often defined with something like JSON schema)
...
them using [`bind_tools()`](https://python.langchain.com/api_reference/core/language_models/langchain_core.language_models.chat_models.BaseChatModel.html#langchain_core.language_models.chat_models.BaseChatModel.bind_tools).
...
[provider reference](/oss/javascript/integrations/providers) for details.

<Tip>

See the [tools guide](/oss/javascript/langchain-tools) for details and other options for
creating tools.
...
The LangGraph framework simplifies the orchestration of model and tool calls.
See the [agent guides](/oss/javascript/langchain-agents) to get started. Below, we show
some simple patterns for illustration.
...
You can pass non-textual data to a model by creating [content blocks](./langchain-messages.mdx#content)
that encapsulate the data. See examples below.
...
2. OpenAI [chat completions](https://platform.openai.com/docs/guides/images-vision?api-mode=chat)
format
...
In these cases, the resulting `AIMessage.content_blocks` will contain blocks
with type `'image'`.
...
see the [integrations page](/oss/javascript/integrations/providers).
...
the [messages](/oss/javascript/python/langchain-messages) guide.
...
`config` parameter using a [`RunnableConfig`](https://python.langchain.com/api_reference/core/runnables/langchain_core.runnables.config.RunnableConfig.html#runnableconfig)
...
[`RunnableConfig`](https://python.langchain.com/api_reference/core/runnables/langchain_core.runnables.config.RunnableConfig.html#runnableconfig)
reference.
