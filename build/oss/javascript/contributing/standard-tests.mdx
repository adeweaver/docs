...
<CardGroup cols={2}>
    <Card title="langchain-core" icon="cube">
        Defines the interfaces we want to import to define our custom components
    </Card>

    <Card title="langchain-tests" icon="flask">
...
<Note>
    We recommend organizing tests in subdirectories:
...
<Tabs>
    <Tab title="Chat models">
        To configure standard tests for a chat model, we subclass `ChatModelUnitTests` and `ChatModelIntegrationTests`. Override these `@property` methods:
        ...
        Additionally, chat model standard tests test a range of behaviors, from basic requirements to optional capabilities like multi-modal support and tool-calling. For a test run to be successful:

        1. **If a feature is supported**: it should pass
        2. **If a feature is not supported**: it should be skipped

        Tests for optional capabilities are controlled via properties that can be overridden. For example, to enable integration tests for image inputs:
...
<Card title="Publish your integration" href="/oss/javascript/contributing/publish" arrow />
