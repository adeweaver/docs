...
Tools are components that [Agents](/oss/python/langchain-agents) call to perform actions. They extend a model capabilities by letting them interact with the world through well-defined inputs and outputs.
...
<Note>
    We recommend that you familiarize yourself with `create_agent()` before covering this section. [Read more agents](/oss/python/langchain-agents).
</Note>
