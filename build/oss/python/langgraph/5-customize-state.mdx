...
<Note>
  This tutorial builds on [Add human-in-the-loop controls](/oss/python/langgraph/4-human-in-the-loop).
</Note>
...
There's one more concept to review before finishing the LangGraph basics tutorials: connecting `checkpointing` and `state updates` to [time travel](/oss/python/langgraph/6-time-travel).
