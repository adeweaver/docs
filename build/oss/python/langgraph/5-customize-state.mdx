...
<Note>
  This tutorial builds on [Add human-in-the-loop controls](/oss/python/4-human-in-the-loop).
</Note>
...
Now, populate the state keys inside of the `human_assistance` tool. This allows a human to review the information before it is stored in the state. Use [`Command`](/oss/python/graph-api#using-inside-tools) to issue a state update from inside the tool.
...
There's one more concept to review before finishing the LangGraph basics tutorials: connecting `checkpointing` and `state updates` to [time travel](/oss/python/6-time-travel).
