...
* How interruptions are resumed in [human-in-the-loop](/oss/python/langgraph/human-in-the-loop) workflows
* How we can "rewind" and branch-off executions using LangGraph's [time travel](/oss/python/langgraph/time-travel) features
...
This architecture is similar to a [ReAct agent](/oss/python/langgraph/agentic-architectures) in which node `"a"` is a tool-calling model, and node `"b"` represents the tools.
