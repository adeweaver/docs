...
First, set up LangGraph API server [locally](/oss/python/local-server) or deploy your agent on [LangGraph Platform](/langgraph-platform/quick-start-studio).
...
Agent Chat UI has full support for [human-in-the-loop](/oss/python/human-in-the-loop) workflows. To try it out, replace the agent code in `src/agent/graph.py` (from the [deployment](/oss/python/local-server) guide) with this [agent implementation](/oss/python/add-human-in-the-loop#add-interrupts-to-any-tool).
