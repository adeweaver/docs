...
<Note>
  This tutorial builds on [Build a basic chatbot](/oss/python/langgraph/1-build-basic-chatbot).
</Note>
...
For the `StateGraph` you created in the [first tutorial](/oss/python/langgraph/1-build-basic-chatbot), add `bind_tools` on the LLM. This lets the LLM know the correct JSON format to use if it wants to use the search engine.
...
The chatbot cannot remember past interactions on its own, which limits its ability to have coherent, multi-turn conversations. In the next part, you will [add **memory**](/oss/python/langgraph/3-add-memory) to address this.
