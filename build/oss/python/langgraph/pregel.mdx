...
LangGraph provides two high-level APIs for creating a Pregel application: the [StateGraph (Graph API)](/oss/python/graph-api) and the [Functional API](/oss/python/functional-api).
...
<Tabs>
  ...
  <Tab title="Functional API">
    In the [Functional API](/oss/python/functional-api), you can use an [`entrypoint`](https://langchain-ai.github.io/langgraph/reference/func/#langgraph.func.entrypoint) to create a Pregel application. The `entrypoint` decorator allows you to define a function that takes input and returns output.
