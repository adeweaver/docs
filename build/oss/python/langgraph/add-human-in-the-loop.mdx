...
To review, edit, and approve tool calls in an agent or workflow, use interrupts to pause a graph and wait for human input. Interrupts use LangGraph's [persistence](/oss/python/persistence) layer, which saves the graph state, to indefinitely pause graph execution until you resume.

<Info>
  For more information about human-in-the-loop workflows, see the [Human-in-the-Loop](/oss/python/human-in-the-loop) conceptual guide.
</Info>

## Pause using `interrupt`

[Dynamic interrupts](/oss/python/human-in-the-loop#key-capabilities) (also known as dynamic breakpoints) are triggered based on the current state of the graph. You can set dynamic interrupts by calling [`interrupt` function](https://langchain-ai.github.io/langgraph/reference/types/#langgraph.types.Interrupt) in the appropriate place. The graph will pause, which allows for human intervention, and then resumes the graph with their input. It's useful for tasks like approvals, edits, or gathering additional context.
...
1. [**Specify a checkpointer**](/oss/python/persistence#checkpoints) to save the graph state after each step.
2. **Call `interrupt()`** in the appropriate place. See the [Common Patterns](#common-patterns) section for examples.
3. **Run the graph** with a [**thread ID**](/oss/python/persistence#threads) until the `interrupt` is hit.
...
2. The `InMemorySaver` is used to store the agent state at every step in the tool calling loop. This enables [short-term memory](/oss/python/add-memory#add-short-term-memory) and [human-in-the-loop](/oss/python/human-in-the-loop) capabilities. In this example, we use `InMemorySaver` to store the agent state in memory. In a production application, the agent state will be stored in a database.
...
To debug and test a graph, use [static interrupts](/oss/python/human-in-the-loop#key-capabilities) (also known as static breakpoints) to step through the graph execution one node at a time or to pause the graph execution at specific nodes. Static interrupts are triggered at defined points either before or after a node executes. You can set static interrupts by specifying `interrupt_before` and `interrupt_after` at compile time or run time.
...
LangGraph Studio is free with [locally deployed applications](/oss/python/local-server) using `langgraph dev`.
