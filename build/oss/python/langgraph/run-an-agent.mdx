...
Agents support both synchronous and asynchronous execution using either `.invoke()` / `await .ainvoke()` for full responses, or `.stream()` / `.astream()` for **incremental** [streaming](/oss/python/langgraph/streaming) output. This section explains how to provide input, interpret output, enable streaming, and control execution limits.
...
Agents use a language model that expects a list of `messages` as an input. Therefore, agent inputs and outputs are stored as a list of `messages` under the `messages` key in the agent [state](/oss/python/langgraph/graph-api#working-with-messages-in-graph-state).
...
<Tip>
  **Using custom agent state**
  ...
  See the [context guide](/oss/python/langgraph/context) for full details.
...
* Optionally, `structured_response` if [structured output](/oss/python/langgraph/quickstart#6-configure-structured-output) is configured.
...
See the [context guide](/oss/python/langgraph/context) for more details on working with custom state schemas and accessing context.
...
<Tip>
  For full details, see the [streaming guide](/oss/python/langgraph/streaming).
</Tip>
