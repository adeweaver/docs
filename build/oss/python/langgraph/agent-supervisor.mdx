...
2. Build a supervisor for orchestrating them with the prebuilt [`langgraph-supervisor`](/oss/python/langgraph/agent-supervisor)
...
To create individual worker agents, we will use LangGraph's prebuilt [agent](/oss/python/langgraph/quickstart).
...
Let's [run the agent](/oss/python/langgraph/run-an-agent) to verify that it behaves as expected.
...
For math agent tools we will use [vanilla Python functions](/oss/python/langgraph/call-tools#define-a-tool):
...
Putting this all together, let's create a graph for our overall multi-agent system. We will add the supervisor and the individual agents as subgraph [nodes](/oss/python/langgraph/graph-api#nodes).
