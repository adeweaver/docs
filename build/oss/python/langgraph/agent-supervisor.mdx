...
2. Build a supervisor for orchestrating them with the prebuilt [`langgraph-supervisor`](/oss/python/agent-supervisor)
...
To create individual worker agents, we will use LangGraph's prebuilt [agent](/oss/python/quickstart).
...
Let's [run the agent](/oss/python/run-an-agent) to verify that it behaves as expected.
...
For math agent tools we will use [vanilla Python functions](/oss/python/call-tools#define-a-tool):
...
Putting this all together, let's create a graph for our overall multi-agent system. We will add the supervisor and the individual agents as subgraph [nodes](/oss/python/graph-api#nodes).
...
Notice that we've added explicit [edges](/oss/python/graph-api#edges) from worker agents back to the supervisor — this means that they are guaranteed to return control back to the supervisor. If you want the agents to respond directly to the user (i.e., turn the system into a router, you can remove these edges).
