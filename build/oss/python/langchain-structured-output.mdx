...
- `ProviderStrategy` for models supporting native structured output (OpenAI GPT models, Grok)
...
When a model incorrectly calls multiple structured output tools, the agent provides error feedback and prompts for a retry:
...
response_format = ToolStrategy(
    schema=ProductRating,
    handle_errors=ValueError  # Only retry ValueError, let others propagate
)
...
response_format = ToolStrategy(
    schema=ProductRating,
    handle_errors=(ValueError, TypeError)  # Retry these specific types
)
```
...
response_format = ToolStrategy(
    schema=ProductRating,
    handle_errors=False  # Errors will crash the agent
)
```
