...
[LLMs](https://en.wikipedia.org/wiki/Large_language_model) are powerful AI tools
that can interpret and generate text like humans. They're versatile enough to
write content, translate languages, summarize, and answer questions
...
call external tools (like databases queries or API calls) and use the results in
their responses.
* <Icon icon="shapes" size={16} /> [Structured output](#structured-outputs) -
where the model's response follows a defined format.
...
`init_chat_model` to initialize one from a [provider](/oss/python/integrations/providers)
of your choice:
...
<CodeGroup>
    ...
    ```
{/* TODO: a code block that has, instead of a language switcher, a provider
switcher to change the code for the respective language to display init_chat
using the respect `model_provider` and `model` name*/}
</CodeGroup>
...
full set of supported parameters vary by model and provider, but standard ones
include:
...
respective provider's [reference docs](/oss/python/integrations/providers).
</Info>
...
A chat model must be invoked to generate an output. There are three primary
...
[`invoke()`](https://python.langchain.com/api_reference/core/language_models/langchain_core.language_models.chat_models.BaseChatModel.html#langchain_core.language_models.chat_models.BaseChatModel.invoke) with a single message or a list of messages.
...
conversation. See the [messages](/oss/python/langchain-messages) guide for more detail
...
Calling [`stream()`](https://python.langchain.com/api_reference/core/language_models/langchain_core.language_models.chat_models.BaseChatModel.html#langchain_core.language_models.chat_models.BaseChatModel.stream) returns an <Tooltip tip="An object that progressively provides access to each item of a collection, in order.">iterator</Tooltip>
...
As opposed to `invoke()`, which returns a single [`AIMessage`](https://python.langchain.com/api_reference/core/messages/langchain_core.messages.ai.AIMessage.html) after
the model has finished generating its full response, `stream()` returns multiple
[`AIMessageChunk`](https://python.langchain.com/api_reference/core/messages/langchain_core.messages.ai.AIMessageChunk.html) objects, each containing a portion of the
output text.
...
<Accordion title="Advanced streaming topics">
    <Accordion title='"Auto-streaming" chat models'>
        ...
        [`astream_events()`](https://python.langchain.com/api_reference/core/language_models/langchain_core.language_models.chat_models.BaseChatModel.html#langchain_core.language_models.chat_models.BaseChatModel.astream_events) to surface the chat
...
improve performance (and reduce costs!), as the processing can be done in
parallel:
...
<Note>
    ...
    model calls client-side.

    It is **distinct** from batch APIs supported by inference providers, such as
    [OpenAI](https://platform.openai.com/docs/guides/batch) or
    [Anthropic](https://docs.anthropic.com/en/docs/build-with-claude/batch-processing#message-batches-api).
</Note>

By default, [`batch()`](https://python.langchain.com/api_reference/core/language_models/langchain_core.language_models.chat_models.BaseChatModel.html#langchain_core.language_models.chat_models.BaseChatModel.batch) will only return the final output
...
[`batch_as_completed()`](https://python.langchain.com/api_reference/core/language_models/langchain_core.language_models.chat_models.BaseChatModel.html#langchain_core.language_models.chat_models.BaseChatModel.batch_as_completed):
...
<Tip>
    ...
    See the [`RunnableConfig`](https://python.langchain.com/api_reference/core/runnables/langchain_core.runnables.config.RunnableConfig.html) reference for a full list of
...
<Note>
    You may hear the term "function calling". We use this interchangeably with
    "tool calling".
</Note>

To make tools that you have defined available for use by a model, you must bind
them using [`bind_tools()`](https://python.langchain.com/api_reference/core/language_models/langchain_core.language_models.chat_models.BaseChatModel.html#langchain_core.language_models.chat_models.BaseChatModel.bind_tools). In subsequent
...
parameters. Check the respective [provider reference](/oss/python/integrations/providers)
for details.

<Tip>
    See the [tools guide](/oss/python/langchain-tools) for details and other options
    for creating tools.
</Tip>
...
[content blocks](/oss/python/langchain-messages#content).
...
See the [integrations page](/oss/python/integrations/providers) for details on specific
providers.
...
[Ollama](/oss/python/integrations/providers/ollama) is one of the easiest ways to run
...
<Accordion title="Base URL" icon="link">
    ...
    <Note>
        ...
        [reference](/oss/python/integrations/providers) for details.
    </Note>
</Accordion>
...
`config` parameter using a [`RunnableConfig`](https://python.langchain.com/api_reference/core/runnables/langchain_core.runnables.config.RunnableConfig.html) dictionary. This
...
[`RunnableConfig`](https://python.langchain.com/api_reference/core/runnables/langchain_core.runnables.config.RunnableConfig.html) reference.
