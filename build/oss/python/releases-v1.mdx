...
**`create_react_agent` has moved from `langgraph` to `langchain`** with significant enhancements:
...
To better support structured output, `create_react_agent` no longer supports pre-bound models with tools or configuration:
