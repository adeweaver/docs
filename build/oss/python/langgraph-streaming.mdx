...
* [**Stream LLM tokens**](/oss/python/streaming#messages) — capture token streams from anywhere: inside nodes, subgraphs, or tools.
* [**Emit progress notifications from tools**](/oss/python/streaming#stream-custom-data) — send custom updates or progress signals directly from tool functions.
* [**Stream from subgraphs**](/oss/python/streaming#stream-subgraph-outputs) — include outputs from both the parent graph and any nested subgraphs.
* [**Use any LLM**](/oss/python/streaming#use-with-any-llm) — stream tokens from any LLM, even if it's not a LangChain model using the `custom` streaming mode.
* [**Use multiple streaming modes**](/oss/python/streaming#stream-multiple-modes) — choose from `values` (full state), `updates` (state deltas), `messages` (LLM tokens + metadata), `custom` (arbitrary user data), or `debug` (detailed traces).
