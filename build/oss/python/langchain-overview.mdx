...
LangChain agents are built on top of [LangGraph](/oss/python/overview). This is done to take advantage of the capabilities LangGraph provides: durable execution, streaming, human-in-the-loop, persistence. You do not need to know LangGraph for basic LangChain agent usage, but some advanced functionality may require LangGraph knowledge.
...
Then, create an agent [using prebuilt components](/oss/python/prebuilts):
...
* [Built on top of LangGraph](/oss/python/overview): LangChain's agents are built on top of LangGraph. This allows us to take advantage of LangGraph's durable execution, human-in-the-loop support, persistence, and more.
