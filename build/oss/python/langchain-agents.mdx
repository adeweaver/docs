...
LangChain's `create_react_agent` function provides a production-ready ReAct (Reasoning + Acting) agent implementation based on the paper [ReAct: Synergizing Reasoning and Acting in Language Models](https://arxiv.org/abs/2210.03629).

<Accordion title="More on ReAct">

ReAct frames an agent's behavior as an interleaving of "Thought -> Action -> Observation" steps, where the model writes out its reasoning, picks a tool, sees the tool's result, and then repeats. By forcing the model to ground its next step on concrete observations, ReAct reduces hallucinations and makes the decision process auditable. The pattern also improves exploration: the agent can form hypotheses ("Thought"), test them with tools ("Action"), and update its plan based on feedback ("Observation"). 
...
To initialize a static model from a model identifier string (requires `langchain` package):
...
Model identifier strings use the format `provider:model` (e.g. `"openai:gpt-4o"`) and support automatic inference (e.g. `"gpt-4o"` will be inferred as `"openai:gpt-4o"`).
...
Model instances give you complete control over configuration. Use them when you need to set specific parameters like temperature, max tokens, timeouts, or configure API keys, base URLs, and other provider-specific settings.
...
<Info>
**`state`**: The data that flows through your agent's execution, including messages, custom fields, and any information that needs to be tracked and potentially modified during processing.
</Info>

Dynamic models are selected at runtime based on the current state and context. This enables sophisticated routing logic and cost optimization.

To use a dynamic model, you need to provde a function that receives the graph state and runtime and returns an instance of `BaseChatModel` with the tools bound to it using `.bind_tools(tools)`, where `tools` is a subset of the `tools` parameter.

```python
from langchain_openai import ChatOpenAI
from langgraph.prebuilt import create_react_agent
from langgraph.graph.message import AgentState, Runtime
...
def select_model(state: AgentState, runtime: Runtime) -> ChatOpenAI:
    ...
    if message_count > 10:
        return ChatOpenAI(model="gpt-4.1").bind_tools(tools)
    else:
...
<Tip>
To learn more about `ToolNode`, see [ToolNode](./langchain-tools#toolnode).
</Tip>
...
result["structured_response"]  # Name: John Doe, Email: john@example.com
```
